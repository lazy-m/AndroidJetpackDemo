 ...
 * 单例数据库对象。请注意，对于真正的应用程序，您可能应该使用依赖关系
 * 注入框架或服务定位器来创建单例数据库。
 */
...
abstract class CheeseDb : RoomDatabase() {
    ...
    /**创建数据库
    *  注意：在实例化CheeseDb对象时，应遵循单例设计模式，因为每个Roomdatabase实例都相当消耗性能，并且您很少需要访问多个实例。
     */
    companion object {
...
//                instance =Room.databaseBuilder(context.applicationContext,CheeseDb::class.java,"CheeseDatabase").build()
            }
            return instance!!
        }
        ...
        private fun fillInDb(context: Context) {
            ...
            //Room中的nserts在当前线程上执行，因此我们在后台插入
            ioThread {
